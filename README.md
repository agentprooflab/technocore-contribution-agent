...
stages a non-editable runtime under `~/.local/share/tca/runtimes/`, smoke-tests it, atomically moves
the `~/.local/share/tca/current` symlink, and registers the observer. The previous target is retained
for rollback. The runtime copy avoids granting a background process access to Documents. Re-run the
installer after upgrading the project.
