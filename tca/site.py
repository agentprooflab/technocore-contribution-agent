...
.controls label{display:block;color:var(--muted);font-size:12px;margin-bottom:10px}.controls input{width:100%;accent-color:var(--cyan)}.budget-number{font-size:44px;font-weight:760;letter-spacing:-.06em}.subtle{color:var(--muted)}.item-list{display:grid;gap:12px}.attention{padding:17px 18px;border:1px solid #2c3553;border-radius:15px;background:#0c1121}.attention[hidden]{display:none}.attention-top{display:flex;justify-content:space-between;gap:12px}.priority{font:700 12px ui-monospace,monospace;color:var(--cyan)}.attention p{margin:10px 0;color:#d5daea}.chips{display:flex;gap:6px;flex-wrap:wrap}.chip{font-size:11px;border:1px solid #35405f;border-radius:999px;padding:4px 8px;color:#afb8d2}
...
def _coverage_cards(context: dict[str, Any]) -> str:
    ...
    for item in context.get("coverage", []):
        ...
        cards.append(
            ...
            f"Known missing {int(item.get('known_missing', 0))}</p>"
            "</article>"
        )
...
def render_site(
    ...
    request_reduction = int(
        evaluation.get("request_model", {}).get("amortized_reduction_basis_points", 0)
    )
    ...
    install_mcp = (
        "uvx --from technocore-contribution-agent tca-mcp\n\n"
...
<header class="shell topbar"><div class="brand">AgentProof<span>/Brief</span></div><div class="badge"><span class="dot"></span> verified build · read only</div></header>
...
<article class="metric"><strong>{request_reduction / 100:.2f}%</strong><span>N=5 request reduction</span></article>
...
<input id="budget" type="range" min="300" max="1800" step="50" value="800"><p class="subtle"><span id="shown">0</span> items fit this view. Runtime accounting uses canonical UTF-8 bytes.</p></aside>
<div class="panel item-list">{_attention_cards(context)}</div></div></section>
...
<div class="proof"><article><b>30/30</b><p>Official-source positives retained.</p></article><article><b>0</b><p>Hard-negative official false positives.</p></article>
